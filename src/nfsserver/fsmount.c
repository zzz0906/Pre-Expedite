...
int loopdev_setup_device(const char *file, uint64_t offset, const char *device)
{
    ...
    if (escalate())
        goto error;
    errno = 0;
    if ((device_fd = open(device, O_RDWR|O_CREAT)) < 0)
    {
...
int IsDirMountPoint(char *dir)
{
    ...
    if (fp == NULL)
    {
        printf("open mount info file: %s error,exit the program!\n", MOUNT_TABINFO);
        return -1;
    }
...
int VFSMount(char *vfs_name, char *vfs_mpoint, const char *vfs_mount_type)
{
    ...
    {
        printf("挂载文件 %s 到loop设备  %s 失败,程序退出!\n", vfs_name, unused_loop_dev);
        return -1;
    }
    ...
    if (vfs_mount_type == NULL || strcmp(vfs_mount_type, "rw") == 0 || (strcmp(vfs_mount_type, "RW") == 0))
    { //初次挂载时默认为读写模式
