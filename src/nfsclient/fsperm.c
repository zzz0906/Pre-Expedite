...
int VFSDirPermission(char *vfs_path, char *perm_mode) // change perm for mount point
{
    mode_t mode = S_IRWXU | S_IRWXG | S_IRWXO;
    errno = 0;                      //reset the errno
    setuid(0);                      //修改文件夹权限时需要进行提权操作
    if (chmod(vfs_path, mode) != 0) //change mode 777.
    {
        printf("修改文件夹 %s[mount point] 权限失败，错误信息为: %s\n", vfs_path, strerror(errno));
        return errno;
    }
